...
M=D`)
    ...
    xit('generates code to perform a `push static` operation', () => {
      const code = generateCode({ type: 'PUSH', arg1: 'static', arg2: 8, _raw: '// push static 8' })
      expect(code).toStrictEqual(
        '')
    })
  })
...
M=D`)
    ...
    xit('generates code to perform a `pop static` operation', () => {
      const code = generateCode({ type: 'POP', arg1: 'static', arg2: 8, _raw: '// pop static 8' })
      expect(code).toStrictEqual(
        '')
    })
  })
